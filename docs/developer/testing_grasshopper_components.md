...
!!! warning "The honest remaining gap"
    ...
    *access-lifting* (with a generic component), and **our own ABB backend code**
    ...
    their existence, but componentizing is not kernel-solving. Solving under a live
    kernel in CI would need a **licensed** Rhino, which is not viable on
    GitHub-hosted runners — so this stays **local-only by design**, and CI's job is
    deliberately to *compile* the nodes, not solve them; (2) the **Tesseract**
...
solve a `GH_Document`, and putting a **licensed** Rhino in GitHub CI is too far to
reach. That boundary is deliberate, not a TODO. What CI *does* verify on Windows
(`build.yml` `build-cpython-components`) is the right half of the split: the `rhino39`
CPython **componentizes** every `Cf_*.ghuser` and the job fails if a required one is
missing. CI compiles the nodes; a local Rhino solves them. The two remaining gaps
are both stated at the top and both by design — kernel-solving compiled components
stays local (licensed Rhino), and the Tesseract env skew is a dependency fact, not a
harness limit.
