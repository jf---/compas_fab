...
- **Backend sources coupled base/tip from the config, not from COMPAS.** A group is *coupled* iff `RobotArtifact.default_inv_kin_solver(group)` names a ROP/REP solver. For a coupled group:
  ...
  - **`plan_cartesian_motion`** sources the coupled group's frames from the config, **topology-aware**: `tcp_frame = manipulator_tip_link` (`tool0`) for both topologies; `working_frame = positioner_tip_link` (`positioner_tool0`, the moving workpiece frame) for **REP**, `positioner_base_link` for **ROP**. `REPInvKin` *rejects* `world`/`positioner_base_link` as working frames — only its positioner tip is valid. Config links are read from the emitted kinematics-plugin YAML — the same authoritative bytes `default_inv_kin_solver` reads — so nothing drifts from the running solver. **Confirmed live (2026-07-25):** on `abb_irb2400_external_positioner`, REP tracks the workpiece-relative target to **1 mm** while the positioner sweeps **1.2 m** on the correct axis (the earlier `positioner_base_link`-for-REP guess was wrong — the silent KDL/scene joint-order swap that the `<joint>`-list group also fixes was the real blocker; see `docs/superpowers/state/w4-e2-rep-coordination-resolved.md`).
...
3. **REP proof decoupled from #6.** The proof uses coupled kinematics + workpiece-relative targets (authored in the `positioner_tool0` frame; no scene-attached rigid body); the `plan_cartesian_motion.py:113` tool/body gate stays shut on the proof path. Attached-workpiece collision handling (#6) lands with its own test, off the proof's critical path.
