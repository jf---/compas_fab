...
from compas_fab.backends.tesseract.errors import InvalidTesseractFreespaceProfileError
from compas_fab.backends.tesseract.freespace_profiles import build_freespace_profiles


def test_unconnected_options_omit_every_native_argument(mocker):
    expected = ProfileDictionary()
    native = mocker.patch(
        "compas_fab.backends.tesseract.freespace_profiles.create_freespace_pipeline_profiles",
        return_value=expected,
    )

    profiles = build_freespace_profiles(None, None, None, None, None, None)
...
def test_supplied_options_are_forwarded_exactly(mocker):
    ...
    build_freespace_profiles(["DEFAULT", "FREESPACE"], 8, 5.0, False, 3, 0.25)
...
def test_explicit_false_optimize_is_forwarded_not_dropped(mocker):
    ...
    build_freespace_profiles(None, None, None, False, None, None)
...
def test_invalid_profile_names_fail(profile_names):
    with pytest.raises(InvalidTesseractFreespaceProfileError):
        build_freespace_profiles(profile_names, None, None, None, None, None)
...
def test_invalid_scalar_options_fail(num_planners, planning_time, optimize, max_solutions, planner_range):
    with pytest.raises(InvalidTesseractFreespaceProfileError):
        build_freespace_profiles(None, num_planners, planning_time, optimize, max_solutions, planner_range)
